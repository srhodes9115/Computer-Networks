...
struct l2pdu* layer2(struct l3pdu* buf, struct l2pdu*buf2, char pos)
{
	...
	if (pos == 'i') //take the data from layer 3 and put it in the layer 2 structure while initailizing layer 2 header and trailer fields
	{
		...
		for(i = 0; i < num_packets; i++)
		{
			...
			datanumber = 120;
			unsigned short int check[sizeof(struct_pointer[i])-6]; //array to count the total sum of the fields: Count, ACK, Seq#, LFrame, Layer3 hdr, and Data Segment
			unsigned short int tempCheck; //used to comvert the unsigned int to an array
			char tempArray[4]; //used to truncate the 5 digit value to 4 digits
			//take all the fields listed above and put them in the array Check
			for (i2 = 0; i2 <3; i2++)
			{
				check[i2] = struct_pointer[i].header.count[i2];
			}
			check[3] = struct_pointer[i].header.ACK;
			check[4] = struct_pointer[i].header.seq_num;
			check[5] = struct_pointer[i].header.Lframe;
			check[6] = struct_pointer[i].layer3.head.ver;
			for (i2 = 7; i2 < 23; i2++)
			{
				check[i2] = struct_pointer[i].layer3.head.src_ipaddr[i2];
			}
			for (i2 = 23; i2 < 39; i2++)
			{
				check[i2] = struct_pointer[i].layer3.head.dest_ipaddr[i2];
			}
			for (i2 = 39; i2 < 46; i2++)
			{
				check[i2] = struct_pointer[i].layer3.head.reserved[i2];
			}
			for (i2 = 0; i2 < datanumber; i2++)
			{
				check[i2+46] = struct_pointer[i].layer3.data_seg[i2];
			}
			tempCheck = checksum(check,sizeof(struct_pointer[i].layer3.data_seg)+46);
			sprintf(tempArray,"%i",tempCheck);
			//shift data to move from 5 digits to 4 digit CheckSum field in layer
			struct_pointer[i].tail.Chksum[0] = tempArray[1];
			struct_pointer[i].tail.Chksum[1] = tempArray[2];
			struct_pointer[i].tail.Chksum[2] = tempArray[3];
			struct_pointer[i].tail.Chksum[3] = tempArray[4];
			printf("\n");
	...
	{ //take the layer 2 structure and set the header/tail fields to null, pass on the data from the input buffer to the current buffer with null fields
		...
		for(i = 0; i <num_packets; i++)
		{
			...
			unsigned short int check[sizeof(struct_pointer[i])-6]; //array to store values in fields used in the checkSUM algorithm 
			unsigned short int tempCheck; //used to convert result of CheckSum function to an array
			char tempArray[4]; //used to truncate 5 digit result of CheckSum algorithm to 4 digits
			...
			if ((tempArray[1] == struct_pointer[i].tail.Chksum[0]) && (tempArray[2] == struct_pointer[i].tail.Chksum[1]) && (tempArray[3] == struct_pointer[i].tail.Chksum[2]) && (tempArray[4] == struct_pointer[i].tail.Chksum[3]))
			{
				printf("\n Checksum successful \n");
			}
			else
			...
			for (i2 = 0; i2<sizeof(struct_pointer[i].layer3.data_seg); i2++)
			{
				...
				if ((struct_pointer[i].layer3.data_seg[i2] == 16) && (struct_pointer[i].layer3.data_seg[i2+1] == 2) || (struct_pointer[i].layer3.data_seg[i2+1] == 3) || (struct_pointer[i].layer3.data_seg[i2+1] == 16))

				{	
					int i3;
					//shift data down one index following a stuffed character
					for(i3 = i2; i3 < sizeof(struct_pointer[i].layer3.data_seg); i3++)
					{
						struct_pointer[i].layer3.data_seg[i3+1] = struct_pointer[i].layer3.data_seg[i3+2];
					}
				}
			}
...
{ 
	if (pos == 'i')
	{
		...
		send_data[datafilesize-1] = 0; //gets rid of end of file character
		send_data[datafilesize-2] = 0; //gets rid of new line character
